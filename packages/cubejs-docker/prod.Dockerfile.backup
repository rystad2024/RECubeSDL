...
# Build all TypeScript packages with production optimizations
RUN yarn build
...
COPY --from=build /cubejs/rust ./rust
...
# Copy the optimized native module (index.node) built with --release
